...
You will receive HARD FACTS — regulatory limits maintained by the system.
...
async def run_compliance_agent(
    ...
    hard_facts = f"""HARD FACTS (regulatory limits):
...
"""
    ...
    try:
        result = await call_claude_json(SYSTEM_PROMPT, user_message, model=MODEL_HAIKU)
